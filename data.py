...
class WordCorpus():
    ...
    def __init__(self, path, freq_cutoff=20, train='train.txt',
    ...
    def tokenize(self, file_name):
        ...
        for line in lines:
            tokens = line.split()
            input_o = get_tag(tokens, 'input')
            word_index = self.word_dict.w2i(get_tag(tokens, 'dialogue'))
            output_g = get_tag(tokens, 'output')
            if output_g[0] not in ['<disagree>', '<disconnect>', '<no_agreement>']:
                o = [int(i) for i in input_o]
                g = [int(i.split('=')[1]) for i in output_g]
                reward = o[1] * g[0] + o[3] * g[1] + o[5] * g[2]
            else:
                reward = 0
            dataset.append((input_o, word_index, output_g, reward))
            # compute statistics
            total += len(word_index)
            unks += np.count_nonzero([idx == unk for idx in word_index])
    ...
    def test_dataset(self, bsz, shuffle=True, device_id=None):
        return self._split_into_batches(copy.copy(self.test), bsz,
                                        shuffle=shuffle, device_id=device_id)
